...
import { resolveRoute, useRouteLocale as useRouteLocale2, withBase } from "vuepress/client";
...
function getSidebar(routePath, routeLocal) {
  ...
  if (_sidebar === "auto") {
    ...
    const dir = Object.keys(_sidebar).sort((a, b) => b.split("/").length - a.split("/").length).find((dir2) => {
      return routePath.startsWith(ensureLeadingSlash(dir2));
    }) || "";
...
function useLocalePostList() {
  ...
  return computed4(() => blogPostData.value.filter((item) => item.lang === locale.value));
}
if (__VUEPRESS_DEV__ && (import.meta.webpackHot || import.meta.hot)) {
...
import { onUnmounted as onUnmounted2, readonly, ref as ref6, watch as watch2 } from "vue";
import { inBrowser as inBrowser2 } from "../utils/index.js";
var focusedElement = ref6();
var active = false;
var listeners = 0;
function useFlyout(options) {
  const focus = ref6(false);
  if (inBrowser2) {
    if (!active) {
      activateFocusTracking();
    }
    listeners++;
    const unwatch = watch2(focusedElement, (el) => {
...
import { computed as computed7, ref as ref7, watch as watch3 } from "vue";
import { useRoute as useRoute3 } from "vuepress/client";
import { normalizeLink as normalizeLink3, resolveNavLink as resolveNavLink2 } from "../utils/index.js";
function useNavbarData() {
  const { theme } = useData();
  return computed7(() => resolveNavbar(theme.value.navbar || []));
}
...
import { inject as inject5, onMounted as onMounted2, onUnmounted as onUnmounted3, onUpdated, provide as provide3, ref as ref8 } from "vue";
...
import { computed as computed11, onMounted as onMounted3, ref as ref9, watchEffect as watchEffect2 } from "vue";
...
function useContributors() {
  const { theme, page, frontmatter } = useData();
  return computed12(() => {
    const showContributors = frontmatter.value.contributors ?? theme.value.contributors ?? true;
    if (!showContributors)
      return null;
...
function useHomeHeroTintPlate(canvas, enable, tintPlate) {
  ...
  function toPlate(plate2) {
    return typeof plate2 === "number" || Number(plate2) === Number.parseInt(plate2) ? [plate2, plate2, plate2].map((n) => Number(n)) : plate2.includes(",") ? plate2.replace(/\s/g, "").split(",").map((n) => Number(n)) : [];
  }
...
function usePostListControl() {
  const { theme } = useData();
  const list = useLocalePostList();
  const blog = computed14(() => theme.value.blog || {});
  const is960 = useMediaQuery3("(max-width: 960px)");
  const postList = computed14(() => {
  ...
  const isLastPage = computed14(() => page.value >= totalPage.value);
  const isFirstPage = computed14(() => page.value <= 1);
  const isPaginationEnabled = computed14(() => blog.value.pagination !== false && totalPage.value > 1);
  const finalList = computed14(() => {
    if (blog.value.pagination === false)
...
import { useRouteLocale as useRouteLocale4 } from "vuepress/client";
...
var tagColorsRef = ref10(articleTagColors);
...
function useLocaleLink(link) {
  const prefix = useRouteLocale3();
  return computed17(() => {
...
function useBlogNavTitle(name) {
  const locale = useRouteLocale4();
  return computed18(() => presetLocales[locale.value]?.[name] || presetLocales["/"][name]);
}
